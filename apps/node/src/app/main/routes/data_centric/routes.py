...
@cross_origin()
def syft_commands():
    """ Receive syft commands and forward routing and forwarding them to a internal worker.
        
        Returns:
            Response: Decoded Syft response generated by the internal worker.
    """
    try:
        # Get message encoding format.
        encoding = request.form["encoding"]
        
        # If message is big, then we should handle with it as a file cause it'll be sent via stream channel.
        if request.files:
            serialized_msg = request.files["payload"].read().decode(self.encoding)
        else:
            # If not, just retrieve the message decoded msg content.
            serialized_msg = request.form["payload"]

        # Encode the message to convert it to binary structure.
        serialized_msg = serialized_msg.encode(encoding)
    
        # Process the message sending it to an internal worker.
        response = local_worker._recv_msg(serialized_msg)
        
        # Decode binary response in order to be able to send it in a json structure.
        decoded_response = response.decode(encoding)
    
    except (EmptyCryptoPrimitiveStoreError,
            GetNotPermittedError,
            ResponseSignatureError,) as e:
        # If any syft exception raises, serialize these exceptions in order to be able to handle with it in the client side.
    ...
    # Add the decoded msg in a json file.
