...
def get_dataset_info_msg(
    ...
    if _allowed:
        storage = node.disk_store
        dataset = storage.get_object_info(_dataset_id)
        _msg = {_dataset_id: dataset}
    else:
        raise AuthorizationError("You're not allowed to get a Dataset!")
...
class DatasetManagerService(ImmediateNodeServiceWithReply):
    ...
    msg_handler_map = {
        ...
        GetDatasetInfoMessage: get_dataset_info_msg,
        GetDatasetsInfoMessage: get_all_datasets_info_msg,
