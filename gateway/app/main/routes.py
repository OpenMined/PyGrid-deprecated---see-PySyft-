...
def join_grid_node():
    ...
    if(data['node-id'] in grid_nodes):
        return Response("This ID has already been registered", status=403, mimetype='appication/json')
...
def get_connected_nodes():
    ...
    return Response(json.dumps({"grid-nodes": list(grid_nodes.keys())}), status=200, mimetype="application/json")
...
def search_dataset_tags():
    ...
    for node in grid_nodes:
        response = requests.post(grid_nodes[node] + "/search", data=json.dumps({'query': body['query']})).content
    ...
    return Response(json.dumps(match_grid_nodes), status=200, mimetype="application/json")
